...
def get_page(url, user_agent=None, verify_ssl=True, proxy=None):
    ...
    # calculate the size of response, to approximate proxy cost
    # try:
    #     size_in_bytes = sys.getsizeof(html)
    #     size_in_kilobytes = size_in_bytes / 1024  # KB
    #     size_in_megabytes = size_in_kilobytes / 1024  # MB
    #     logging.info(
    #         f"Size of the response: {size_in_bytes} bytes, {size_in_kilobytes} KB, {size_in_megabytes} MB")
    # except Exception as e:
    #     logging.error(f"Error in calculating the size of the response: {e}")
...
def search(query, tld='com', lang='en', tbs='0', safe='off', num=10, start=0,
    ...
    while not stop or count < stop:
        ...
        # print(
        #     f"Time taken by the google search function : {function_end_time - function_start_time}")
